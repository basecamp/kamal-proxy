...
func TestHostWriterMap_SetAndGet(t *testing.T) {
	t.Run("using domain names", func(t *testing.T) {
		hwm := NewHostWriterMap(0, filepath.Join(t.TempDir(), "hwm"))
	...
	t.Run("with significant path segments", func(t *testing.T) {
		hwm := NewHostWriterMap(2, filepath.Join(t.TempDir(), "hwm"))
...
func TestHostWriterMap_Persistence(t *testing.T) {
	persistPath := filepath.Join(t.TempDir(), "hwm")

	hwm := NewHostWriterMap(2, persistPath)
