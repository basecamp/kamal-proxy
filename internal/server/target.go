...
type TargetOptions struct {
	...
	CookiePathPrefix    bool              `json:"cookie_path_prefix"`
}
...
func (t *Target) SendRequest(w http.ResponseWriter, req *http.Request) {
	...
	tw := newTargetResponseWriter(w, inflightRequest, t.cookieScope(req), req.URL.Hostname())
...
func (t *Target) cookieScope(r *http.Request) string {
	if !t.options.CookiePathPrefix {
		return ""
	}
	...
	return routingContext.MatchedPrefix
}
...
func newTargetResponseWriter(w http.ResponseWriter, inflightRequest *inflightRequest, cookieScope, cookieDomain string) *targetResponseWriter {
...
func (w *targetResponseWriter) scopeCookies() {
	if w.cookieScope != "" {
		cookies := w.header["Set-Cookie"]
		for i, v := range cookies {
			cookie, err := http.ParseSetCookie(v)
			if err != nil || (cookie.Domain != "" && cookie.Domain != w.cookieDomain) {
				continue
			}

			cookie.Path, err = url.JoinPath(w.cookieScope, strings.Trim(cookie.Path, "/"))
			if err == nil {
